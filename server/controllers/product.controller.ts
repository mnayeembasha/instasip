    ...
    try {
        ...
        for (const img of images) {
            if (img && img.startsWith('data:image/')) {
                ...
                processedPublicIds.push(uploadResult.public_id);
            }
        }
        ...
        if (processedImages.length > 0) {
            ...
            imagePublicId = processedPublicIds[0];
        }
        const newProduct = await Product.create({
            ...
            imagesPublicIds: processedPublicIds,
            stock,
            category
        });
...
export const deleteProduct = async (req: Request, res: Response) => {
    ...
    try {
        ...
        for (const pid of existingProduct.imagesPublicIds) {
            if (pid) {
                try {
